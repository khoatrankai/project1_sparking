...
import { IGetWork, IUpdateWork } from "@/models/activityInterface";
...
export default function ModalUpdateWork({ ID }: Props) {
  ...
  const fetchData = async () => {
    ...
    if (res.statusCode === 200) {
      const dataRes = res.data as IGetWork;
      form.setFieldsValue({
        ...dataRes,
  ...
  const handleSubmit = async (values: IUpdateWork) => {
    const res = await postdata(() => activityService.updateWork(ID, values));
    if (res === 200 || res === 201) {
      dispatch(fetchWorks());
      setIsModalVisible(false);
    }
