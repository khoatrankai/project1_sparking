...
const ModalOriginalProduct = () => {
  ...
  const handleSubmitEdit = async (value: string, id: string, index: number) => {
    try {
      const statusCode = await postdata(() =>
        productService.updateType(id, value)
      );
        ...
        return (
          <>
            {indexEdit.includes(index) ? (
              <Form
                onFinish={(e) => {
                  handleSubmitEdit(e.name, record.original_id, index);
                }}
                style={{
                  display: "flex",
                  flexWrap: "wrap",
              ...
              >
                <Form.Item
                  ...
                  style={{ margin: "0" }}
                >
                  <Input placeholder="Nhập tên xuất xứ" defaultValue={value} />
                </Form.Item>
                <Form.Item
                  style={{
                    gap: "2px",
  ...
  const handleSubmit = async (values: IOriginal) => {
    try {
      const statusCode = await postdata(() =>
        productService.createOriginal(values)
      );
...
export default ModalOriginalProduct;
