import { ICreateActivityContainer, IGetActivityContainer, IUpdateActivityContainer } from '@/models/productInterface';
...
export const fetchActivityContainers = createAsyncThunk(
  ...
  async (type:'import'|'export', thunkAPI) => {
    try {
...
export const fetchActivityContainerById = createAsyncThunk(
  ...
  async (id: string, thunkAPI) => {
    try {
      ...
      return thunkAPI.rejectWithValue(`Failed to fetch activity container with ID ${id}`);
    }
  }
);
...
export const updateActivityContainer = createAsyncThunk(
  ...
  async ({ id, data }: { id: string; data: IUpdateActivityContainer }, thunkAPI) => {
    try {
      const response = await productService.updateActivityContainer(id, data);
      return response;
    } catch  {
      return thunkAPI.rejectWithValue(`Failed to update activity container with ID ${id}`);
    }
  }
);
    ...
    builder
      ...
      .addCase(updateActivityContainer.fulfilled, (state, action) => {
        ...
        const index = state.datas.findIndex(
          (container) => container.activity_container_id === action.payload.data.activity_container_id
        );
        if (index >= 0) {
