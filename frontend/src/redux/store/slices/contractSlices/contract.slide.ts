...
import { ICreateContract, IGetContract, IUpdateContract } from '@/models/contractInterface';
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateContract.fulfilled, (state, action) => {
        const index = state.datas.findIndex((item) => item.contract_id === action.payload.data.contract_id);
        if (index !== -1) {
